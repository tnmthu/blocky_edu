...
addFormulae = function (object, attributes, code, declareParams, callerParams, callerMethod, ref, callbackKeyword = "callback") {
    ...
    */
    ...
    for (var index = 0; index < listblock.length; index++) {
        ...
        if (name.length != 0) {
            name = Blockly.Python.variableDB_.getName(name, Blockly.Variables.NAME_TYPE);
    ...
    if (ref.hasOwnProperty('module')) {
        var declareString = ref.getFieldValue('PORT') + ' = ' + ref.library + '.' + ref.module.slice(0, 1) + ref.module.substring(1).toLowerCase() + '(board.' + ref.getFieldValue('PORT') + ')';
...
addAction = function (object, isAwait, callMethod, params) {


}

addGetter = function (object, isAwait, callMethod, params) {
    /*
        object : object name for calling : PORT1 , PORT2
        attributes : unique attribute to generate function name : PORT1_onClick_1
        declareParams : parameter to put inside function declare : async def PORT1_onClick_1( someVariabe )
...
// var controller
updateAllPortDropdown = function () {
        ...
        if (block.getField('PORT') != null) {
            if (block.type == 'pxt.core.portDeclare') {
                ...
                if (pos > 0) {
                    ...
                    block.getInput('MAIN').insertFieldAt(pos, new Blockly.FieldDropdown(portDropdown), 'PORT');
...
function translateWidgetDemand(block) {
    ...
    var hexColor = Blockly.Python.valueToCode(block, "COLOUR", Blockly.Python.ORDER_ATOMIC).slice(1, -1);
    ...
    switch (block.widget) {
        ...
        case "BUTTON":
            ...
            // Mode of operation
            template["pushMode"] = ((block.getFieldValue("MODE") == "PUSH") ? true : false);
            // Size of button 
        ...
        case "NUMBER":
            ...
            var prefix = Blockly.Python.valueToCode(block, "PREFIX", Blockly.Python.ORDER_ATOMIC).slice(1, -1);
            var size = block.getFieldValue("DIMENSION");
        ...
        case "CHART":
            ...
            for (var u = 0; u < listChildBlock.length; u++) {
                ...
                if (childBlock.type != "pxt.widget.superchart.channel.declare") continue;


                var stream = LOAD_TEMPLATE("STREAM");
                stream["title"] = Blockly.Python.valueToCode(childBlock, "WIDGETNAME", Blockly.Python.ORDER_ATOMIC).slice(1, -1);
                ...
                var streamHexColor = Blockly.Python.valueToCode(childBlock, "COLOUR", Blockly.Python.ORDER_ATOMIC).slice(1, -1);
    ...
    // Name of widget
    template["label"] = Blockly.Python.valueToCode(block, "WIDGETNAME").slice(1, -1);
    template["tabId"] = parseInt(block.getRootBlock().getFieldValue("TAB").substring(3, 4)) - 1;
...
function getWidgetDemand() {
    ...
    for (var i = 0; i < listBlock.length; i++) {
        ...
        tabWidget["tabs"][tabId]["label"] = Blockly.Python.valueToCode(block, "NAME", Blockly.Python.ORDER_ATOMIC);
        // Remove the double quote
        tabWidget["tabs"][tabId]["label"] = tabWidget["tabs"][tabId]["label"].substring(1, tabWidget["tabs"][tabId]["label"].length - 1);
    ...
    for (var i = 0; i < listBlock.length; i++) {
        ...
        if (widget) {
            if (widget["type"] != "ENHANCED_GRAPH") {
                ...
                currentChannel += 1;
                assignedChannel["V" + String(currentChannel)] = [block, widget, widget["label"]];
                ...
                for (var u = 0; u < widget["dataStreams"].length; u++) {
                    currentChannel += 1;
                    assignedChannel["V" + String(currentChannel)] = [block, widget, widget["dataStreams"][u]["title"]]
...
function exec_checkRequestWidget() {
    ...
    if (JSON.stringify(nowDemand) != prevDemand) {
        ...
        if (codelab.isWidgetChanged == false && prevDemand.length != 0) {
            ...
            //     type: "info"
            // })
            codelab.toast.showSuccess("You have made changes to to your widgets, remember to press the UPLOAD button after you are done!")
...
changeHandle = function (targetBlock, change) {
...
handleUiChange = function (targetBlock, change) {

}

var controller_port_assign = {};

getListBlockType = function (type, workspace) {
    var listBlock = [];
...
getListAvailablePort = function (module) {
    var listPort = [];
...
__argumentList__ = function (targetBlock) {
    var myInputList = [];
    // //LEVEL=1 console.log(targetBlock);
    for (var inputIndex = 0; inputIndex < targetBlock.inputList.length; inputIndex++) {
        for (var fieldIndex = 0; fieldIndex < targetBlock.inputList[inputIndex].fieldRow.length; fieldIndex++) {
            myInputList.push(targetBlock.inputList[inputIndex].fieldRow[fieldIndex].text_);
        }
    }
...
__updatePortService__ = function () {

}


function debugEvent(msg) {
    console.warn('%c' + msg, 'color : #000 ;font-size : 10px ;font-family:"Comic Sans MS"')
...
for (var i = 0; i < _supported_port.length; i++) __port__[_supported_port[i]] = null;
    ...
    */
    ...
    if (POLICY_DISABLE_ORPHANS) {
        for (var i = 0; i < blocks.length; i++) {
            ...
            if (block.getRootBlock() == block && block.previousConnection != null && block.outputConnection == null) {
            ...
            else if (block.getRootBlock() == block && block.outputConnection != null && block.previousConnection == null) {
                if (true) {
    ...
    if (POLICY_PORT_CONTROLLER) {
        ...
        for (var i = 0; i < blocks.length; i++) {
            ...
            if (__port__[block.getFieldValue('PORT')] != null) {
                listDisabled.push(block);
                block.setWarningText(block.getFieldValue('PORT') + ' has been connected to ' + __port__[block.getFieldValue('PORT')]);
        ...
        for (var i = 0; i < blocks.length; i++) {
            ...
            */
            ...
            if (block.type != "pxt.widget.superchart.declare") {
                ...
                myName = Blockly.Python.valueToCode(block, "WIDGETNAME", Blockly.Python.ORDER_ATOMIC).slice(1, -1);
        ...
        for (var i = 0; i < blocks.length; i++) {
            ...
            var dropdownObject = new Blockly.FieldDropdown(generateDropdown(dropdown));
            var pos = block.getInput('MAIN').fieldRow.indexOf(block.getField('PORT'));
            var selectedPort = block.getFieldValue('PORT');
            if (pos > 0) {
                block.getInput('MAIN').removeField('PORT');
        ...
        for (var i = 0; i < blocks.length; i++) {
            ...
            if (dropdown.indexOf(currentPort) == -1) {
                listDisabled.push(block);
                block.setWarningText(block.module + ' is no longer connected to ' + currentPort);
    ...
    if (false) {
        for (var i = 0; i < blocks.length; i++) {
            ...
            for (var u = 0; u < blocks.length; u++) {
                ...
                var thisName = Blockly.Python.valueToCode(blocks[u], "WIDGETNAME", Blockly.Python.ORDER_ATOMIC).slice(1, -1);
                listUsedName.push(thisName);
            }

            var myName = Blockly.Python.valueToCode(block, "WIDGETNAME", Blockly.Python.ORDER_ATOMIC).slice(1, -1);
    ...
    if (POLICY_WIDGET_CONTROLLER) {
        ...
        for (var i = 0; i < blocks.length; i++) {
            block = blocks[i];
            if (["widget.set", "widget.get", "widget.event"].indexOf(block.blockType) == -1) continue;

            var currentWidgetName = block.getFieldValue("WIDGETNAME");
            var channelDropdown = [];
            if (widgetChannel[block.widget] == undefined || widgetChannel[block.widget].length == 0) {
                channelDropdown = [
                    ["NO WIDGET", "NO WIDGET"]
                ];
                listDisabled.push(block);
            ...
            //LEVEL=1 console.log('dropdown', block, portDropdown)
            console.log(block)
            var pos = block.getInput('MAIN').fieldRow.indexOf(block.getField('WIDGETNAME'));
            if (pos > 0) {
                block.getInput('MAIN').removeField('WIDGETNAME');
                block.getInput('MAIN').insertFieldAt(pos, new Blockly.FieldDropdown(channelDropdown), 'WIDGETNAME');
    ...
    if (false) {
        ...
        for (var i = 0; i < blocks.length; i++) {
            ...
            var myChannelName = Blockly.Python.valueToCode(block, "WIDGETNAME", Blockly.Python.ORDER_ATOMIC).slice(1, -1);
        ...
        for (var i = 0; i < blocks.length; i++) {
            ...
            if (listDisabled.indexOf(block) > -1) continue;
            // if (block.disabled) continue;
            if (block.blockType != "widget.set" && block.blockType != "widget.get" && block.blockType != "widget.event") continue;

            if (!(block.widget in channelName) || channelName[block.widget].length == 0) {
                listDisabled.push(block);
                ...
                if (pos > 0) {
                    block.getInput('MAIN').removeField('WIDGETNAME');
                    block.getInput('MAIN').insertFieldAt(pos, new Blockly.FieldDropdown(portDropdown), 'WIDGETNAME');
    ...
    for (var i = 0; i < blocks.length; i++) {
        ...
        if (listDisabled.indexOf(block) > -1) continue;
        if ((block.previousConnection != null && block.getRootBlock() != block) ||
